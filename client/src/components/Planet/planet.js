...
function positionForMoon(planet, time) {
...
function positionForPlanet(planet, time) {
	const yearDuration = planet.info.yearDuration * 60 + 1;
...
function animation( time, planet, speedTicks ) {
	...
	if (planet.info.parentPlanet) {
		positionForMoon(planet, time, speedTicks);
	} else {
		positionForPlanet(planet, time, speedTicks);
	}
